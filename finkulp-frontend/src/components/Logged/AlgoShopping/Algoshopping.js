          ...
          <Table sx={{ minWidth: 300 }} aria-label="simple table">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="right" style={{ fontWeight: '600', fontSize: '14px', padding: '8px' }}>Know More</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {filteredAlgos.map((algo, index) => (
                <TableRow key={index} style={{ fontSize: '12px', height: '40px', backgroundColor: index % 2 === 0 ? '#FFFFFF' : '#F3F6FF' }}>
                  ...
                  <Link to='/loggedhome/MarketPlace/AlgoDescription'><button className='bg-blue dark:bg-dark-2 border-dark dark:border-dark-2 border rounded-full inline-flex items-center justify-center py-2 px-4 text-center text-sm font-medium text-white hover:bg-body-color hover:border-body-color disabled:bg-gray-3 disabled:border-gray-3 disabled:text-dark-5' onClick={()=>{props.setViewAlgo(algo)}}>
                      Open
                    </button></Link>
                  </TableCell>
                </TableRow>
